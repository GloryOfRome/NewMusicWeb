...
namespace NewMusicWeb.Controllers
{
    public class ArtistsController : Controller
    {
        ...
        public ActionResult Edit(int? id)
        {
            ...
            foreach (var item in db.Songs.Where(t=>t.ArtistId==id).ToList())
            {
                Song s = new Song();
                s.Id = item.Id;
                s.Name = $"{item.Name}, 被购买数：{db.Boughts.Where(t => t.SongId == item.Id).ToList()?.Count()}";

                list.Add(s);
            }


            ViewBag.ArtistId = new SelectList(list, "Id", "Name");
